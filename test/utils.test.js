...
describe("Utils", () => {
  ...
  describe("#getPrivateKey12", () => {
    it("empty alias", () => {
      assert.throws(() => {
        utils.getPrivateKey({keyStore : "./test/res/keys/pkcs12/test_key_container.p12"});
      }, /Key alias is not set/);
    });

    it("empty password", () => {
      assert.throws(() => {
        utils.getPrivateKey({keyStore : "./test/res/keys/pkcs12/test_key_container.p12", keyStoreAlias:"keyalias"});
      }, /Keystore password is not set/);
    });

    it("valid p12", () => {
      const pk = utils.getPrivateKey({keyStore: "./test/res/keys/pkcs12/test_key_container.p12", keyStoreAlias:"mykeyalias", keyStorePassword:"Password1"});
      assert.ok(pk);
    });

    it("valid p12, alias not found", () => {
      assert.throws(() => {
        utils.getPrivateKey({keyStore:"./test/res/keys/pkcs12/test_key_container.p12", keyStoreAlias:"mykeyalias1", keyStorePassword:"Password1"});
  ...
  describe("#getPrivateKeyPem", () => {
    it("valid pkcs8 pem", () => {
      const pk = utils.getPrivateKey({keyStore:"./test/res/keys/pkcs8/test_key.pem"});
      assert.ok(pk);
    });

    it("valid pkcs1 pem", () => {
      const pk = utils.getPrivateKey({keyStore:"./test/res/keys/pkcs1/test_key.pem"});
      assert.ok(pk);
    });
  ...
  describe("#getPrivateKeyDer", () => {
    it("valid pkcs8 der", () => {
      const pk = utils.getPrivateKey({keyStore:"./test/res/keys/pkcs8/test_key.der"});
      assert.ok(pk);
    });
