...
function Crypto(config) {
  ...
  this.publicKeyFingerprint = config.publicKeyFingerprint || publicKeyFingerprint.call(this, this.encryptionCertificate);
  ...
  this.encryptData = function (options, encryptionParams) {
    ...
    if (this.publicKeyFingerprintFieldName) {
...
/**
 * Get SHA-256 fingerprint from a X509 certificate
 *
 * @private
...
function isValidConfig(config) {
  ...
  let propertiesHeader = ["ivHeaderName", "encryptedKeyHeaderName", "oaepHashingAlgorithmHeaderName", "publicKeyFingerprintHeaderName"];
  let contains = (props) => {
