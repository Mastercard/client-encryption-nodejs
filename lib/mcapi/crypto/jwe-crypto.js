...
function JweCrypto(config) {
  ...
  this.encryptData = function (options) {
    ...
    const encodedJweHeader = toEncodedString(JSON.stringify(jweHeader),c.UTF8, c.BASE64URL);
    ...
    const encodedEncryptedSecretKey = toEncodedString(encryptedSecretKey, c.BINARY, c.BASE64URL);
    const encodedIv = toEncodedString(iv, c.BINARY, c.BASE64URL);
    const encodedEncryptedText = toEncodedString(cipherText, c.BASE64, c.BASE64URL);
  ...
  this.decryptData = function (encryptedData) {
    ...
    const decipher = nodeCrypto.createDecipheriv(decryptionEncoding, secretKey, Buffer.from(iv, c.BINARY), {authTagLength: authTagBinary.length});
    if (gcmMode === true) {
...
function computePublicFingerprint(config, encryptionCertificate) {
  if (config && encryptionCertificate) {
    return utils.computePublicFingerprint(config, forge.pki.certificateFromPem(encryptionCertificate), c.BASE64);
  } else {
    return null;
  }
}
